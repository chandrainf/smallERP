...
class DeklarasiToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=deklarasi">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: deklarasiSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: deklarasiSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(DeklarasiToolbar);
