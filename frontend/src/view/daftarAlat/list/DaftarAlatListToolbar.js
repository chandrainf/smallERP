...
class DaftarAlatToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=daftarAlat">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: daftarAlatSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: daftarAlatSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(DaftarAlatToolbar);
