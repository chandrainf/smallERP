...
class SupplierViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    ...
    return (
      <div key={record.id}>{record['namaSupplier']}</div>
    );
  };
...
export default connect(select)(SupplierViewItem);
