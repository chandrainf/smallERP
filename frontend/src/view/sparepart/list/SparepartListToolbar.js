...
class SparepartToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=sparepart">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: sparepartSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: sparepartSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(SparepartToolbar);
