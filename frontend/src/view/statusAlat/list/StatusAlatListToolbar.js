...
class StatusAlatToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=statusAlat">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: statusAlatSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: statusAlatSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(StatusAlatToolbar);
