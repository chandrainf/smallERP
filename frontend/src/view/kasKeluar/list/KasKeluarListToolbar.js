...
class KasKeluarToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=kasKeluar">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: kasKeluarSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: kasKeluarSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(KasKeluarToolbar);
