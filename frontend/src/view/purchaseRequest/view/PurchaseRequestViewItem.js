...
class PurchaseRequestViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link to={`/purchaseRequest/${record.id}`}>
            {record['nama']}
          </Link>
        </div>
      );
...
PurchaseRequestViewItem.propTypes = {
...
export default connect(select)(PurchaseRequestViewItem);
